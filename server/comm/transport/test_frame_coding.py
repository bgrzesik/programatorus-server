...
class FrameEncoderTest(unittest.TestCase):
    ...
    def test_write_with_zeros(self):
        ...
        self._test_encoding(
                [0x1, 0x0, 0x1],
                [0x81, 0x80, 0x1, 0x82, 0x80, 0x1, 0x0]
        )
        self._test_encoding(
                [0x1, 0x0, 0x1, 0x0],
                [0x81, 0x80, 0x1, 0x81, 0x80, 0x1, 0x0]
        )
...
class FrameDecoderTest(unittest.TestCase):
    ...
    def test_stream_without_zeros(self):
        self._test_single_frame(
                [0x1],
                [0x00, 0x82, 0x80, 0x1, 0x0]
        )
        self._test_single_frame(
                [0x1, 0x2],
                [0x00, 0x83, 0x80, 0x1, 0x2, 0x0]
        )
        self._test_single_frame(
                [0x1, 0x2, 0x3],
                [0x00, 0x84, 0x80, 0x1, 0x2, 0x3, 0x0]
        )

    def test_steam_with_zeros(self):
        self._test_single_frame(
                [0x1, 0x0],
                [0x00, 0x81, 0x80, 0x1, 0x0]
        )
        self._test_single_frame([0x0], [0x0, 0x80, 0x80, 0x0])
        self._test_single_frame(
                [0x1, 0x0],
                [0x0, 0x81, 0x80, 0x1, 0x0]
        )
        self._test_single_frame(
                [0x1, 0x0, 0x1],
                [0x0, 0x81, 0x80, 0x1, 0x82, 0x80, 0x1, 0x0]
        )
        self._test_single_frame(
                [0x1, 0x0, 0x1, 0x0],
                [0x0, 0x81, 0x80, 0x1, 0x81, 0x80, 0x1, 0x0]
        )
